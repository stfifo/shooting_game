...
function initBackground() {
  ...
  for (let i = 0; i < 180; i++)
    STARS.push({ x: rnd(0, W), y: rnd(0, H), r: rnd(.5, 2), spd: rnd(5, 20), br: rnd(.3, 1) });

  for (let i = 0; i < 14; i++)
    BG_OBJS.push({ type: 'cloud',  x: rnd(0, W), y: rnd(-H, H), w: rnd(50, 140), h: rnd(18, 45),  spd: rnd(30, 80) });
  for (let i = 0; i < 18; i++)
    BG_OBJS.push({ type: 'rock',   x: rnd(0, W), y: rnd(-H, H), r: rnd(4, 18),   spd: rnd(25, 65), rot: rnd(0, Math.PI * 2), rs: rnd(-.8, .8) });
  for (let i = 0; i < 6; i++)
    BG_OBJS.push({ type: 'nebula', x: rnd(0, W), y: rnd(-H, H), r: rnd(55, 110),  spd: rnd(12, 28), col: pick(['rgba(80,20,160,', 'rgba(180,20,60,', 'rgba(20,80,160,']) });
  for (let i = 0; i < 16; i++)
    BG_OBJS.push({ type: 'ice',    x: rnd(0, W), y: rnd(-H, H), r: rnd(5, 16),    spd: rnd(18, 50), rot: rnd(0, Math.PI * 2), rs: rnd(-.5, .5) });
  for (let i = 0; i < 5; i++)
    BG_OBJS.push({ type: 'galaxy', x: rnd(40, W - 40), y: rnd(-H, H), r: rnd(40, 80), spd: rnd(8, 20), col: pick(['rgba(180,100,255,', 'rgba(100,150,255,', 'rgba(255,150,100,']) });
  for (let i = 0; i < 10; i++)
    BG_OBJS.push({ type: 'alien',  x: rnd(0, W), y: rnd(-H, H), r: rnd(8, 24),    spd: rnd(20, 55) });
}

/** 현재 웨이브 기준 목표 배경 페이즈 반환 */
...
/** 현재 웨이브 기준 배경 스크롤 속도 반환 (px/s) */
function bgScrollSpeed() {
  if (wave <= 4)  return 20;
  if (wave <= 9)  return 42;
  if (wave <= 14) return 68;
  if (wave <= 19) return 100;
  if (wave <= 24) return 130;
  if (wave <= 29) return 165;
  return 200;
}
...
function drawBG() {
  ...
  const G0 = GRADS[ph] || GRADS[6];
  const G1 = GRADS[Math.min(6, ph + 1)];
  ...
  if (cloudA > 0.02) {
    BG_OBJS.filter(o => o.type === 'cloud').forEach(c => {
      cx.globalAlpha = cloudA * .35;
      cx.fillStyle   = '#fff';
      drawPixelCloud(c.x, c.y, c.w, c.h);
    });
    cx.globalAlpha = 1;
    if (ph === 0) drawPixelTerrain();
  }
  ...
  if (nebA > 0.01) {
    BG_OBJS.filter(o => o.type === 'nebula').forEach(n => {
      ...
      cx.fillStyle = gg2;
      cx.fillRect(n.x - n.r, n.y - n.r, n.r * 2, n.r * 2);
    });
  }
  ...
  if (uranA > 0.01) {
    cx.globalAlpha = uranA;
    drawPixelUranus(W - 72, 100, 42);
    drawPixelNeptune(58, 175, 34);
    cx.globalAlpha = 1;
  }

  // 명왕성 + 카이퍼 벨트 얼음 (phase 4)
  const plutoA = clamp(ph + bgBlend - 3.8, 0, 1) * clamp(5.5 - ph - bgBlend, 0, 1) * .85;
  if (plutoA > 0.01) {
    cx.globalAlpha = plutoA;
    drawPixelPluto(W - 58, 148, 30);
    cx.globalAlpha = 1;
    BG_OBJS.filter(o => o.type === 'ice').forEach(r => {
      cx.globalAlpha = plutoA * .9;
      cx.save();
      cx.translate(r.x, r.y);
      cx.rotate(r.rot);
      drawPixelIceRock(r.r);
      cx.restore();
    });
    cx.globalAlpha = 1;
  }
  ...
  if (alienA > 0.01) {
    ...
    glow.addColorStop(0, 'rgba(0,100,0,0)');
    glow.addColorStop(1, `rgba(0,140,30,${alienA * .5})`);
    cx.fillStyle = glow;
    cx.fillRect(0, 0, W, H);

    BG_OBJS.filter(o => o.type === 'alien').forEach(a => {
      cx.globalAlpha = alienA * .9;
      drawAlienPod(a.x, a.y, a.r);
    });
    cx.globalAlpha = 1;
  }
}
...
function drawPixelTerrain() {
  const S = 4;
  const strips = [
    { y: H - 28, col: '#2a5518', h: 8 },
    { y: H - 20, col: '#3a6a28', h: 6 },
    { y: H - 14, col: '#4a7a38', h: 5 },
    { y: H - 9,  col: '#5a8a48', h: 5 },
    { y: H - 4,  col: '#6a9a58', h: 4 },
  ];
  strips.forEach(s => {
    cx.fillStyle = s.col;
    for (let x = 0; x < W; x += S)
      if (Math.random() > .08) cx.fillRect(x, s.y, S, s.h);
  });
}

function drawPixelRock(r) {
  const S  = 2;
  const ir = Math.ceil(r / S);
  cx.fillStyle = '#6a5a48';
  for (let y = -ir; y <= ir; y++)
    for (let x = -ir; x <= ir; x++)
...
function drawPixelJupiter(px, py, r) {
  ...
  for (let y = -ir; y <= ir; y++) {
    ...
    for (let x = -ir; x <= ir; x++) {
      if (x * x + y * y >= ir * ir) continue;
      cx.fillStyle = bands[bIdx % bands.length];
      cx.fillRect(px + x * S, py + y * S, S, S);
    }
  }
  // 대적점
...
function drawPixelUranus(px, py, r) {
  ...
  for (let y = -ir; y <= ir; y++) {
    for (let x = -ir; x <= ir; x++) {
      if (x * x + y * y >= ir * ir) continue;
      const n = (x + ir) / (ir * 2);
      cx.fillStyle = n > .6 ? '#88ddcc' : n > .3 ? '#55bbaa' : '#33998a';
      cx.fillRect(px + x * S, py + y * S, S, S);
    }
  }
  // 고리
  cx.globalAlpha = .3;
  cx.fillStyle   = '#aaeedd';
  for (let rx = -(ir + 5); rx <= (ir + 5); rx++)
    if (Math.abs(rx) > ir * .9) cx.fillRect(px + rx * S - 1, py - 2, 2, 4);
  cx.globalAlpha = 1;
}
...
function drawPixelNeptune(px, py, r) {
  ...
  for (let y = -ir; y <= ir; y++) {
    for (let x = -ir; x <= ir; x++) {
      if (x * x + y * y >= ir * ir) continue;
      const n = (y + ir) / (ir * 2);
      cx.fillStyle = n > .65 ? '#1a4888' : n > .35 ? '#1a3870' : '#0e2858';
      cx.fillRect(px + x * S, py + y * S, S, S);
    }
  }
  cx.fillStyle = '#2a58a8';
  for (let x = -1; x <= 1; x++) cx.fillRect(px + x * S - S, py + S, S, S);
}

function drawPixelPluto(px, py, r) {
  const S   = 2;
  const ir  = Math.ceil(r / S);
  const hs  = ir * 0.45;  // Tombaugh Regio 하트 크기
  const hcy = ir * 0.08;  // 하트 중심 Y 오프셋
...
function drawPixelIceRock(r) {
  ...
  for (let y = -ir; y <= ir; y++) {
    for (let x = -ir; x <= ir; x++) {
      if (x * x * .9 + y * y >= ir * ir * .85) continue;
      const n = (x + y + ir * 2) / (ir * 4);
      cx.fillStyle = n > .65 ? '#cce8f0' : n > .4 ? '#99bbcc' : '#6688aa';
...
function drawGalaxy(gx, gy, gr, col) {
  for (let i = 0; i < 80; i++) {
    ...
    cx.globalAlpha = 0.15 + i / 200;
    cx.fillStyle   = col + '0.7)';
...
function drawAlienPod(px, py, r) {
  ...
  for (let y = -ir; y <= ir; y++) {
    for (let x = -ir; x <= ir; x++) {
      if (x * x + y * y >= ir * ir) continue;
      const n = (x + y + ir * 2) / (ir * 4);
      cx.fillStyle = n > .65 ? '#44cc44' : n > .4 ? '#228822' : '#116611';
      cx.fillRect(px + x * S, py + y * S, S, S);
    }
  }
  ...
  if (Math.floor(performance.now() / 900) % 7 !== 0) {
    cx.fillStyle = '#aaffaa';
    cx.fillRect(px - S * 2, py - S, S, S);
    cx.fillRect(px + S,     py - S, S, S);
  }

  // 녹색 발광 후광
  const gg = cx.createRadialGradient(px, py, 0, px, py, r * 2.5);
  gg.addColorStop(0, 'rgba(0,200,50,.12)');
  gg.addColorStop(1, 'rgba(0,0,0,0)');
  cx.fillStyle = gg;
  cx.fillRect(px - r * 3, py - r * 3, r * 6, r * 6);
}
