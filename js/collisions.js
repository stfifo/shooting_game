...
function killEnemy(m,bomb=false){
  ...
  if(!bomb){const skillGain=wave>15?3:1;skillKills=Math.min(SKILL_GOAL,skillKills+skillGain);if(skillKills>=SKILL_GOAL&&P&&P.weapon!==5)addFx('SKILL READY [X]',W/2,80,'#ff0',13);}
...
function hitPlayer(){
  ...
  rapidT=0;pierceT=0;
  if(!godMode)lives--;P.invT=2.5;if(!fireballT){P.weapon=1;P.wpTimer=0;}P.fireT=.5;
