...
/**
 * 비보스 웨이브 랜덤 생성.
 * 티어(1~6)에 따라 편대 수·적 종류·대형·진입 경로를 무작위 선택.
 */
function randWave(n) {
  ...
  // wave 1: 튜토리얼 난이도 — A형 소형 편대 1개, TOP 진입
  if (n === 1) {
...
/** 웨이브 시작: 보스 웨이브는 경고 연출 후 스폰, 일반 웨이브는 랜덤 편대 */
function launchWave(n) {
  ...
  if (isBossWave(n)) {
    startBossWarning(n); // 3.5초 경고 후 보스 스폰
...
function updateWaves(dt) {
  // 웨이브 간 대기: 타이머 소진 후 다음 웨이브 시작
  if (betweenWave) {
    betweenT -= dt;
    if (betweenT <= 0) {
      betweenWave = false;
      if (waveIdx < TOTAL_WAVES) launchWave(waveIdx + 1);
    }
    return;
  }
  ...
  // wave clear 판정
  // bossWarnActive 중에는 판정 금지 — 호위 편대 전멸해도 경고 연출 끝날 때까지 대기
