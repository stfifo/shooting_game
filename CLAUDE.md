...
Strikers 1945+ 스타일의 종스크롤 슈팅 게임. 빌드 도구 없이 **`game.html` 단일 파일**로 구성된다. 브라우저에서 직접 열면 바로 실행된다.
...
```
assign3/
├── game.html             # 게임 전체 (HTML + CSS + JS 단일 파일)
├── backups/              # 자동 백업 (001_game_YYYYMMDD_HHmmss.html)
├── report.md             # 과제 보고서
└── .claude/
...
`<script>` 내부가 섹션 주석(`// ── 이름 ───`)으로 구분된다. 위에서 아래로 선언 순서가 의존 순서이기도 하다.
